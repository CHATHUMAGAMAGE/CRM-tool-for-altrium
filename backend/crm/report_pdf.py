...
def _visual_rows(report_name: str, payload: dict[str, Any]):
    if report_name == "lead-sources":
        ...
        rows = [{"label": "Total Leads", "value": values["total_leads"]}, {"label": "Leads Proceeding", "value": values["proceed"]}, {"label": "Deals Created", "value": values["deals"]}]
    maximum = max((row["value"] for row in rows), default=0)
    total = sum(row["value"] for row in rows) if report_name == "lead-sources" else 0
    visual_rows = []
    for row in rows:
        row_percentage = percentage(row["value"], total) if total else None
        visual_rows.append({
            **row,
            "width": percentage(row["value"], maximum) if maximum else 0,
            "percentage": row_percentage,
            "display_value": (
                f'{row["value"]} ({row_percentage}%)'
...
def build_pdf_context(report_name: str, payload: dict[str, Any], filters: dict[str, Any], user):
    ...
    return {
        ...
        "performance_keep_together": 0 < len(performance_rows) <= 8,
...
def _generate_reportlab_pdf(context: dict[str, Any]) -> bytes:
    ...
    metadata = [
        ...
        [safe(context["period"]), [safe(context["generated_by"]), safe(f'Role: {context["generated_role"]}', small)], safe(f'{context["generated_at"].strftime("%d %B %Y, %H:%M")} ({context["timezone_name"]})')],
    ]
    ...
    for row in context["visual_rows"]:
        bar_width = document.width * .56
        drawing = Drawing(bar_width, 7)
        drawing.add(Rect(0, 0, bar_width, 7, fillColor=colors.HexColor("#EDF2F8"), strokeColor=None))
        if row["width"] > 0:
            drawing.add(Rect(0, 0, bar_width * row["width"] / 100, 7, fillColor=blue, strokeColor=None))
        visual = Table([[safe(row["label"]), drawing, safe(row["display_value"])]], colWidths=[document.width * .29, document.width * .56, document.width * .15])
    ...
    if context["performance_rows"]:
        ...
        performance_section = [Paragraph("Source Performance", heading), performance_table]
        story.extend([KeepTogether(performance_section)] if context["performance_keep_together"] else performance_section)
    ...
    details = [
        ...
        [safe("Generated By", small), safe(context["generated_by"]), safe("Role", small), safe(context["generated_role"])],
        [safe("Generated At", small), safe(f'{context["generated_at"].strftime("%d %B %Y, %H:%M")} ({context["timezone_name"]})'), "", ""],
